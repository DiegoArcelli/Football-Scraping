...
# /scrape and /regions launch Chromium headed (WhoScored blocks/challenges
# headless browsers), so this needs a virtual display to run inside a
# container. Start Xvfb directly (xvfb-run's SIGUSR1 readiness handshake
# isn't reliable in all container runtimes) and give it a moment to come up.
CMD Xvfb :99 -screen 0 1280x1024x24 -nolisten tcp & sleep 1 && DISPLAY=:99 ./api
